...
x <- data.frame(
  price = x$total_pr,
  cond_new = ifelse(x$cond == "new", 1, 0),
  stock_photo = ifelse(x$stock_photo == "yes", 1, 0),
  duration = x$duration,
  wheels = x$wheels
)
...
## univariate regression, removed outliers
lm2 <- lm(price ~ cond_new, data = x1)
summary(lm2)
hist(lm1$residuals)

## multivariate regression
lm3 <- lm(price ~ cond_new + stock_photo + duration + wheels, data = x1)
summary(lm3)
hist(lm3$residuals)

## model selection, drop duration
lm4 <- lm(price ~ cond_new + stock_photo + wheels, data = x1)
...
## summarize model 4
summary(lm4)
lm5 <- lm(price ~ cond_new + wheels, data = x1)

## slight worsening in model performance
summary(lm5)$adj.r.squared - summary(lm4)$adj.r.squared


### check out model residuals
par(mar = c(4.5, 4.5, 1, 1))
plot(predict(lm4), abs(lm4$residuals),
     ylab = "Abs. Value of Residuals",
     xlab = "Fitted Values",
     cex.lab = 1.5, cex.axis = 1.25,
     col = "blue", pch = 19)
abline(lm(abs(lm4$residuals) ~ predict(lm4)), lty = 2, lwd = 2)
...
$$
\text{transformation}(p_i) = \beta_0 + \beta_1 x_1 + ... + \beta_k x_k
$$

$$
logit(p_i) = log_e \bigg( \dfrac{p_i}{1 - p_i} \bigg) = ln \bigg( \dfrac{p_i}{1 - p_i} \bigg)
$$

$$
ln \bigg( \dfrac{p_i}{1 - p_i} \bigg) = \beta_0 + \beta_1 x_1 + ... + \beta_k x_k
$$

$$
p_i = \dfrac{e^{\beta_0 + \beta_1 x_1 + ... + \beta_k x_k}}{1 + e^{\beta_0 + \beta_1 x_1 + ... + \beta_k x_k}}
$$
...
y$logit <- log(y$p / (1 - y$p))
head(y)

plot(y$logit, y$p, type = "l", lwd = 2,
     xlab = "logit(p)", ylab = "p",
     cex.axis = 1.25, cex.lab = 1.5)
abline(h = c(0, 1), lty = 2, col = "grey", lwd = 2)

```

### Example 1: Audit Study

![](images/aer.png)

![](images/names.png)

![](images/descriptions_3.png)

```{r}

## read in data
x <- read.csv("data/resume.csv")

## simplify
x <- data.frame(
  callback = x$received_callback,
  chicago = ifelse(x$job_city == "Chicago", 1, 0),
  college_degree = x$college_degree,
  years_experience = x$years_experience,
  military = x$military,
  honors = x$honors,
  email_address = x$has_email_address,
  white = ifelse(x$race == "white", 1, 0),
  male = ifelse(x$gender == "m", 1, 0)
)
...
## run, analyze regression
lm1 <- glm(callback ~ honors, data = x, family = binomial(link = "logit"))
summary(lm1)

## coefficients
b0 <- as.numeric(coefficients(lm1)[1])
b1 <- as.numeric(coefficients(lm1)[2])
coefs <- b0 + b1

## prob. of callback if no honors
exp(b0) / (1 + exp(b0))

## prob of callback if honors
exp(coefs) / (1 + exp(coefs))

## impact of honors
### this is not equal to b1!
exp(coefs) / (1 + exp(coefs)) - exp(b0) / (1 + exp(b0))

## run a full model
lm2 <- glm(callback ~ ., data = x, family = binomial(link = "logit"))
summary(lm2)

## how can it be that college degree is not significant?
glm(honors ~ college_degree, data = x, family = binomial(link = "logit"))

## aggregate
a <- aggregate(honors ~ college_degree, data = x,
               function(x) c(mean = mean(x),
                             sd = sd(x),
                             n = length(x)))
a <- as.data.frame(do.call(cbind, a))
a$min <- a$mean - 1.96 * a$sd / sqrt(a$n)
a$max <- a$mean + 1.96 * a$sd / sqrt(a$n)
a <- a*100
a$college_degree <- a$college_degree / 100
a

## barplot
b <- barplot(a$mean, ylim = c(0, 8),
        ylab = "Honors Share (%)", 
        cex.lab = 1.5, cex.axis = 1.25)
axis(1, at = b, labels = c("No College", "College"), cex.axis = 1.25)
arrows(b, a$min, b, a$max, angle = 90, code = 3, length = 0.05, lwd = 2)

## best model
lm3 <- glm(callback ~ ., data = x[,-3], family = binomial(link = "logit"))
summary(lm3)

## estimate the probability of receiving a callback for:
### a job in Chicago
### 14 years experience
### no honors, no military experience,
### email address and has a first name that implies they are a White male

z <- as.data.frame(coefficients(lm3))
z <- data.frame(
  var = row.names(z),
  coef = z[,1]
)
z$x <- c(1, 1, 14, 0, 0, 1, 1, 1)
z

coefs <- sum(z$coef * z$x)
prob_callback <- exp(coefs) / (1 + exp(coefs))
prob_callback

## female premium
z$x <- c(1, 1, 14, 0, 0, 1, 1, 0)
z
coefs <- sum(z$coef * z$x)
prob_callback_f <- exp(coefs) / (1 + exp(coefs))
prob_callback_f - prob_callback

## black penalty
z$x <- c(1, 1, 14, 0, 0, 1, 0, 1)
z
coefs <- sum(z$coef * z$x)
prob_callback_b <- exp(coefs) / (1 + exp(coefs))
prob_callback_b - prob_callback

## predict
x$fitted_values <- predict(lm3, family = binomial(link = "logit"))

## convert to probabilities
x$fitted_values <- exp(x$fitted_values) / (1 + exp(x$fitted_values))

## aggregate
a <- aggregate(fitted_values ~ callback, x, 
          function(x) c(mean = mean(x),
                        sd = sd(x),
                        n = length(x)))
a <- as.data.frame(do.call(cbind, a))
...
b <- barplot(a$mean, ylim = c(0, 10),
             ylab = "Predicted Prob. (%)", 
             cex.lab = 1.5, cex.axis = 1.25)
axis(1, at = b, labels = c("No Callback", "Callback"), cex.axis = 1.25)
arrows(b, a$min, b, a$max, angle = 90, code = 3, length = 0.05, lwd = 2)

## our prediction is imperfect, but accurate
minz <- min(x$fitted_values)
maxz <- max(x$fitted_values)
loops <- seq(minz, maxz, 0.05)
n_loops <- length(loops) - 1
y <- list()
for(i in 2:n_loops){
  z <- x[x$fitted_values >= loops[i-1] & x$fitted_values < loops[i],]
  y[[length(y)+1]] <- data.frame(
    midpoint = loops[i-1] + (loops[i] - loops[i-1])/2,
    mean = mean(z$callback),
    sd = sd(z$callback),
    n = nrow(z)
  )
}
...
plot(x$fitted_values, x$callback,
     ylab = "Callback (Truth)", xlab = "Predicted Probability",
     cex.lab = 1.5, cex.axis = 1.25, pch = 19, col = "blue")
abline(0, 1, lty = 2, lwd = 2)
points(y$midpoint, y$mean, pch = 19)
arrows(y$midpoint, y$min, y$midpoint, y$max,
       angle = 90, code = 3, length = 0.05, lwd = 2)
